...
class UnhandleQuery(Fault):                                  #无法处理的查询异常情况
    def __init__(self, message="Couldn't handle the query"):
        Fault.__init__(self, UNHANDLED, message)


class AccessDenied(Fault):                                   #用户访问未被授权的资源时出现的异常情况
    def __init__(self, message="Acces denied"):
        Fault.__init__(self, ACCESS_DENIED, message)


def inside(dir, name):                                       #检查给定目录中是否有给定的文件名
    dir = abspath(dir)
    name = abspath(name)
    return name.startswith(join(dir, ''))


def getPort(url):                                            #从url中提取端口号
    name = urlparse(url)[1]
    parts = name.split(':')
    return int(parts[-1])


class Node:                                                  #P2P网络中的节点。
